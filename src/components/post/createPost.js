...
export default function CreatePost({ setShowModal, showModal, tokenId }) {
  ...
  const [ tipp, setTip ] = useState(null);
	...
	const [totSupply, setTotSupply] = useState(tokenId);
  ...
  // const [ id, setid ] = useState('');
    ...
    const networkData = Tip.networks[ networkId ];
      ...
      setTip(tip);
			// const supply = await mint.methods.getNumberOfTokensMinted().call();
			// console.log(supply);
			setTotSupply(totSupply + 1);
		} else {
	...
	const onChangeOUrl = (e) => {
		setourl(e.target.value);
	};

	const onChangeGUrl = (e) => {
							...
							<div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
								...
								<div className="relative p-6 flex-auto">
									<form onSubmit={handleSubmit}>
										...
										<input
											aria-label="Enter your OpenSea URL"
											type="text"
											placeholder="Enter OpenSea URL"
											className="text-sm text-gray-base w-full mr-3 py-5 px-4 h-2 border border-gray-primary rounded mb-2"
											// required
											onChange={onChangeOUrl}
										/>
										<input
										...
										/>
										<input
											aria-label="Enter your Google Photo URL"
											type="text"
											// required
											placeholder="Enter your Google Photo URL"
											className="text-sm text-gray-base w-full mr-3 py-5 px-4 h-2 border border-gray-primary rounded mb-2"
											onChange={onChangeGUrl}
										/>
									</form>
								</div>
								{/* footer */}
